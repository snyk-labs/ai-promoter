...
class Share(db.Model):
    ...
    post_content = db.Column(db.Text, nullable=False)  # The actual content that was posted
    post_url = db.Column(db.Text)  # URL to the post if available
    
    # Relationships
    user = db.relationship('User', backref=db.backref('shares', lazy='dynamic', cascade='all, delete-orphan'))
    ...
    def get_platform_share_counts(cls, content_id):
        ...
        ).filter_by(content_id=content_id).group_by(cls.platform).all() 
...
class RedisClient:
    ...
    def init_app(self, app):
        ...
        redis_kwargs = app.config.get(
            "REDIS_CONNECTION_KWARGS", {}
        ).copy()  # Get from config
        ...
        if not redis_url:
            app.logger.warning(
                f"RedisClient: REDIS_URL is not configured. Client will not be initialized."
            )  # Updated log
            self.client = None
            return

        app.logger.info(
            f"RedisClient: Attempting to connect using REDIS_URL: {redis_url} with options: {redis_kwargs}"
        )  # Updated log
        ...
        try:
            ...
            )  # Use new redis_kwargs
            self.client.ping()  # Verify connection by pinging
            app.logger.info(
                f"RedisClient: Successfully connected to Redis at {redis_url} and received PONG."
            )  # Use redis_url directly

        except redis.exceptions.ConnectionError as e:
            app.logger.error(
                f"RedisClient: Failed to connect to Redis at '{redis_url}'. ConnectionError: {e}"
            )  # Use redis_url directly
            self.client = None  # Ensure client is None on connection failure
        except Exception as e:
            app.logger.error(
                f"RedisClient: An unexpected error occurred while connecting to Redis at '{redis_url}': {e}"
            )  # Use redis_url directly

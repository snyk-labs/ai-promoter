...
def scrape_content_task(self, content_id: int, url: str):
    ...
    try:
        ...
        if not updated_content:
            ...
            raise Exception(f"Failed to process content from URL {url} for content_id {content_id}")
            
        logger.info(f"Successfully processed and updated content {content_id} from URL {url}")
        return content_id
        
    except Exception as e:
        logger.error(f"Error scraping content_id {content_id} (URL: {url}): {str(e)}. Retry {self.request.retries + 1}/{self.max_retries}.")
